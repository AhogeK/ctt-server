...
additionally supports `type=DEVICES` / `type=IDES` (per-device / per-IDE seconds; IDE buckets derive from the origin device's registered IDE name), unknown or foreign devices yield 404);
achievements

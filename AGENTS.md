...
**例外情况** (立即执行，无需二次确认):
用户明确指令包含以下关键词时**直接执行**，禁止二次确认：

- "提交"、"commit"
- "推送"、"push"
- "做吧"、"继续"
- **连续指令**: "提交然后..."、"commit and then..."、"提交并做 X"

**连续指令处理原则**:
当用户说"提交现在的代码，然后做 X"时，这是单一连续指令，应该：
1. **立即提交**当前变更
2. **继续执行**后续任务 X
3. **禁止**提交后再问"是否提交"

**禁止擅自提交的场景** (必须明确询问):
- "优化覆盖率" / "添加测试" / "修复测试" → ❌ 写测试即可，不要提交
- "代码审查" / "review" → ❌ 只报告问题，不要提交
- "处理 X 问题" / "优化 Y" → ❌ 只执行任务，不要提交
- 任何没有明确出现"提交"、"commit"的指令 → ❌ 先询问
